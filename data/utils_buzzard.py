...
def get_df_true(true_mag_file, true_pos_file, despzbands_file):
    ...
    df_true_pos = load_fits_file(true_pos_file, columns=['ID', 'Z', 'RA', 'DEC', 'SIZE', 'TILE', 'REDSHIFT FIELD'] + columns_add_cardinal) # 'INDEX'
    df_true_pos.rename({'RA' : 'RA_ORIGINAL', 'DEC' : 'DEC_ORIGINAL'}, inplace=True, axis='columns')

    print('loading despz information')
    df_true_mag = load_fits_file(true_mag_file, columns=['LMAG']) # 'FLUX',
    df_despzbands = pd.read_csv(despzbands_file,header=None) # , skiprows=0
    despzbands = [b[0].split('.')[0].split('/')[-1] for b in df_despzbands.values]
    #print(df_true_mag.shape)
    #pdb.set_trace()
    df_true_mag.columns = ['TRUEMAG_' + b for b in despzbands] # ['TRUEFLUX_' + b for b in despzbands] + 
